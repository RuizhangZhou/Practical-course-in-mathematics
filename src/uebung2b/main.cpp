...
void read(ifstream &ifs, vector<T> &array) {
    auto t = make_unique<T>();//"make_unique" is similar to "new"
    //and don't need to delete()
    //create a variable t and the datatype is <T>
    while (ifs >> *t) {//what's the meaning of this condition?
    //if ifs ist not null, then *t=ifs?
